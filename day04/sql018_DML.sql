...
-- NULL값이 지정 가능한 속성은 생략 가능
-- 단, 값을 생략하면 컬럼을 지정해줘야 함
...
-- 특수 INSERT
-- Improted_Book 테이블의 데이터 자체를 Book에 그대로 추가
INSERT INTO Book(bookid, bookname, publisher, price)
...
-- 수정(UPDATE)
-- 수정/삭제는 항상 트랜잭션과 같이 실행

-- 사전 준비
-- 이전의 Customer 테이블 내용을 CustomerNew로 복사
